...
object CaseIndex {
    def main(args: Array[String]) {
        ...
        all_files.foreach(file=>{
            ...
            val locations = wordProcess()
            val people = wordProcess()
            val organizations = wordProcess()
            ...
            sentence_list.foreach(e_sentence=>{
                var parameterNLP = Http("""http://localhost:9000/?properties=%7B'annotators':'ner','outputFormat':'json'%7D""").postData(e_sentence).method("POST").header("Content-Type", "application/json").option(HttpOptions.connTimeout(60000)).option(HttpOptions.readTimeout(60000)).asString.body
            
                // parse reponse to JSON object
                val NLP_json: JsValue = Json.parse(parameterNLP)
                
                // get all entities
                val tokens = NLP_json \\ "tokens"
                tokens.foreach(token=>{
                    val text = token; val ner = token; var idx = 0;
                    ...
                    while (idx < text.length){
                        if(ner(idx).toString == "\"PERSON\""){
                            people = people + text(idx).toString
                        } else if(ner(idx).toString == "\"LOCATION\""){
                            locations = locations + text(idx).toString
                        } else if(ner(idx).toString == "\"ORGANIZATION\""){
                            organizations = organizations + text(idx).toString
                        }
                        idx = idx + 1
                    }   
                })
